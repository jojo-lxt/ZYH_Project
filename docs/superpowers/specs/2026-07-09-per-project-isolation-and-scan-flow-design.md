...
1. 用户手动在数据库注册(无需绑定默认项目);登录后可创建多个项目,项目归创建者所有(`properties.owner_id`)。~~注册时绑定一个项目(`console_users.property`)~~ —— 后续迭代已移除 `console_users.property` 字段,权限改由 `properties.owner_id` 决定;`role` 收敛为「管理员」「游客」两种并加 `CHECK` 约束。
