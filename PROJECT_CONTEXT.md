...
8. 小程序做半自动闭环：把图片存进手机相册、文案复制到剪贴板，再引导用户去小红书发布（选相册图 + 粘贴文案）。
9. 用户在平台 App 中最终确认发布。
...
- 小程序已接入预览接口 + 发布存档 + 半自动发布闭环(`xhs-miniprogram/utils/xhsPublish.js`:存图到相册 + 复制文案 + 引导手动发布);小红书未开放小程序端笔记预填发布,想「一键预填」需官方放开原生跳转或改用原生 App 分享 SDK;小程序改动需在小红书/微信 IDE 真机预览自测(需在后台配 `downloadFile` 合法域名)
