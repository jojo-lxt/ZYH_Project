...
# 可选:混合思考模型(DeepSeek V4 等)关思考。文案这种结构化任务不需要推理,关掉更快、也不会让思考吃掉
# max_tokens 预算导致 JSON 被截断(captionReason=truncated)。取值 disabled / enabled;不配则不下发该字段
# (给通义/混元/Kimi 等不认识 thinking 参数的厂商发未知字段可能 400,所以默认不传)。
